  ...
  methods: {
    ...
    drawCoordinateLine() {
      ...
      this.leftTitle.map((index, item) => {
        ...
        this.ctx.strokeStyle = this.xlineColor;
        if (index == 2) {
    ...
    removeDot(e) {
      if (e.offsetX && e.offsetY) {
        ...
        this.dotList.map((item, index) => {
          if (Math.abs(item.x - dot.x)< 5 && Math.abs(item.y - dot.y) < 5) {
            isResult = true;
            replaceIndex = index;
          }
        });
    ...
    onMouseEvent(e){
       if (e.offsetX && e.offsetY) {
        ...
        let dot = this.correctDotPosition(x, y);
          let xIndex = parseInt((dot.x - this.spanX) / this.spanX);
        let yIndex = parseInt(
          (this.canvasHeight - dot.y - this.spanY) / this.spanY
        );   
        let t = {
          x:xIndex,
          y:yIndex,
